...
从 [GitHub Releases][github-release-link] 下载：

- **普通版 zip**：解压后在 `chrome://extensions`（Edge 用 `edge://extensions`）开启“开发者模式”→ “加载已解压的扩展程序”→ 选择 `read-frog-local` 文件夹。
- **双击启动版**：解压后每次从 `启动ReadFrog-Chrome.bat` / `启动ReadFrog-Edge.bat` 启动浏览器，无需开发者模式。
